...
```{r get_shifted_overlap}
ln4_overlap_shift <- get_overlap_counts(ln4_files, tss_windows, offset = 112)

ln5_overlap_shift <- get_overlap_counts(ln5_files, tss_windows, offset = 112)
```

```{r save_new_values}
overlap_values_shift <- overlap_values
overlap_values_shift[names(ln5_overlap_shift), "ln5"] <- ln5_overlap_shift
overlap_values_shift[names(ln4_overlap_shift), "ln4"] <- ln4_overlap_shift
overlap_values_shift[, "ln4_frac"] <- overlap_values_shift[, "ln4"] / total_reads["ln4"]
overlap_values_shift[, "ln5_frac"] <- overlap_values_shift[, "ln5"] / total_reads["ln5"]
```

```{r shifted_correlations}
out_cor_shift <- lapply(seq(1, nrow(all_comb)), function(i_row){
  print(i_row)
  correlate_non_zero(overlap_values_shift, as.character(all_comb[i_row,]), log_transform = TRUE, test = TRUE)
})
out_cor_shift <- do.call(rbind, out_cor_shift)
rownames(out_cor_shift) <- all_comb_names
out_cor_shift

out_graphs_shift <- lapply(seq(1, nrow(all_comb)), function(i_row){
  use_vars <- as.character(all_comb[i_row,])
  subpoints <- subsample_nonzeros(overlap_values_shift, use_vars, non_zero = "both", n_points = 10000)
  ggplot(subpoints, aes_string(x = use_vars[1], y = use_vars[2])) + geom_point() + scale_y_log10() + scale_x_log10()
})
```
