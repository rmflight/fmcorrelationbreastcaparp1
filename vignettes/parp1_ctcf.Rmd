...
% \VignetteIndexEntry{fmanalysisbreastcaparp1: CTCF Binding}
% \VignetteDepends{GenomicRanges, fmdatabreastcaparp1}
% \VignettePackage{fmanalysisbreastcaparp1}
-->

# Overlap with CTCF Chip-Seq within Transcription Start Sites
...
```{r read_ctcf}
ctcf_names <- c("chrom", "start", "end", "name", "score", "strand", "signal", "pvalue", "qvalue", "peak")
ctcf_path <- file.path(data_dir, "ctcf_peaks")
ctcf_files <- file.path(ctcf_path, dir(ctcf_path, pattern = "narrowPeak"))
...
```{r graphit}
r1_v_ln4 <- subsample_nonzeros(mcols(tss_windows), c("ctcf_r1", "ln4_read"), non_zero = non_zero, n_points = 10000)
ggplot(r1_v_ln4, aes(x = ctcf_r1, y = ln4_read)) + geom_point() + scale_y_log10() + scale_x_log10()
