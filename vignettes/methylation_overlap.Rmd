...
methyl_data <- lapply(methyl_files, function(x){
  ...
  rm(tmp)
  methyl_vals <- mclapply(names(tss_windows), function(in_window){
    methyl_overlap <- subsetByOverlaps(methyl_tmp, tss_windows[in_window], minoverlap = 80, ignore.strand = TRUE)
    average_value(methyl_overlap, "mcols.signal")
  })
})
