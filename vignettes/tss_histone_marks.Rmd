...
Based on previous manuscripts, it may be a good idea to restrict both the histone and parp1 reads to a 2kb window surrounding annotated transcription start sites (tss). 
...
Now we read in the histone marks data.
...
histone_data <- mclapply(histone_files, function(x){
...
```{r histone_sample_dataframe}
n_tss <- length(tss_windows)
overlap_values <- DataFrame(matrix(0, nrow = n_tss, ncol = length(histone_data) + 4), row.names = names(tss_windows))

colnames(overlap_values) <- c(names(histone_data), "ln4", "ln5", "ln4_frac", "ln5_frac")
...
```{r grab_all}
all_comb <- expand.grid(names(histone_data), c("ln4", "ln4_frac", "ln5", "ln5_frac"), stringsAsFactors = FALSE)
