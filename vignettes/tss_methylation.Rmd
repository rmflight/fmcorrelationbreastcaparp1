...
```{r graphit}
methyl_v_ln4 <- subsample_nonzeros(mcols(tss_methyl_parp1), c("methyl", "ln4"), non_zero = "both", n_points = 10000)
ggplot(methyl_v_ln4, aes(x = methyl, y = ln4)) + geom_point() + scale_y_log10() + scale_x_log10()
...
```{r grab_all}
all_comb <- expand.grid("methyl", c("ln4", "ln5"), stringsAsFactors = FALSE)
