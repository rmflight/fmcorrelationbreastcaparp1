...
In doing some of the exploratory analysis and examining replicates, a bunch of the regions in the PARP1 data have low counts, ~ 10, where it appears that the correlative structure breaks down due to an additive noise component. This can be seen in plotting the log intensities of the replicates against each other (need to use logs because of the proportional error). Ideally, we might be able to simply provide a minimum value to the function that calculates the correlations, but we first need to be able to justify the minimum value that we are using. To do that, we need to be able to show where the correlation breaks down. Although initially seen in the PARP1 data, we will also do a similar analysis for all the data sets that consist of replicates, including methylation and CTCF binding.
