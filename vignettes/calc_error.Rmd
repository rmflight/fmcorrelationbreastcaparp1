...
parp1_data_split <- split(parp1_data, parp1_split)
...
Note that this is not quite right. What we actually need to do is cut the data in both the X and Y, and get out these little chunks that fit that range in both directions. Need a better way, cause what I did above is not correct. Need to fix it. Hunter has suggested using the mean value across replicates as a way to delineate the points that should go together.

Did we do the residuals correctly? The actual numbers are correct, but the residuals within a given range?

Alternatively, what is the variance in Y values for binned values of X, and what is the variance in X values for binned values of Y? At some point in the X and Y, there should be stabilization of the values, i.e. the variance will get smaller or stabilize. I'm not sure anymore.

## Bootstrap the Orthogonal Regression by Decile, and adding Deciles

What if instead, we did a bootstrap of the orthogonal regression in each decile? What would the variance of the slope look like in each of the deciles?

```{r boot_odregress}
parp1_data_split_index <- split(seq(1, nrow(parp1_data)), parp1_split) 

parp1_boot_odregress <- mclapply(parp1_data_split_index, function(in_split){
				 bootstrap_odregress(parp1_data[in_split,1], parp1_data[in_split,2])
})
